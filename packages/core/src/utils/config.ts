...
export const generateConfig = (
  ...
  const colors = (oldConfig as any).colors;
  ...
  if (oldConfig.isV2 === true) {
    ...
    if (isValidHexColor(colors.primary)) {
      designTokens['colors']['brand'] = getBrandColors(colors.primary);
    } else if (typeof colors.primary === 'object') {
      designTokens['colors']['brand'] = colors.primary;
    }
    if (isValidHexColor(colors.secondary)) {
      designTokens['colors']['background'] = getBackgroundColorsOld(colors.secondary);
    } else if (typeof colors.secondary === 'object') {
      designTokens['colors']['background'] = colors.secondary;
    }
    if (isValidHexColor(colors.text)) {
      designTokens['colors']['text'] = colors.text;
    }
    if (isValidHexColor(colors.textPrimary)) {
      designTokens['colors']['text-on-brand'] = colors.textPrimary;
    }
    if (isValidHexColor(colors.videoBackground)) {
      designTokens['colors']['video-bg'] = colors.videoBackground;
    }
  }
